...
void Camera::Initialize(const glm::vec3& position, const glm::vec3& target, const Camera::Data& data)
{
	...
	m_mxView = glm::lookAt(transform.m_position, m_target, glm::vec3(0.0f, 1.0f, 0.0f));
	transform.m_rotation = glm::quat_cast(m_mxView);
...
void Camera::UpdateTransformEditor(const glm::vec3& translate, const glm::vec3& rotate)
{
	...
	transform.m_rotation = qpitch * transform.m_rotation * qyaw;
	transform.m_rotation = glm::normalize(transform.m_rotation);

	transform.m_position += (translate * transform.m_rotation);

	glm::mat4 mxTranslation = glm::translate(glm::mat4(1.0), -transform.m_position);
	glm::mat4 mxRotation = glm::mat4_cast(transform.m_rotation);
...
void Camera::UpdateTransformOrbit(const glm::vec3& translate, const glm::vec3& rotate)
{
	glm::quat qpitch = glm::angleAxis(-rotate.x, glm::vec3(1.0f, 0.0f, 0.0f));
	glm::quat qyaw = glm::angleAxis(-rotate.y, glm::vec3(0.0f, 1.0f, 0.0f));
	transform.m_rotation = transform.m_rotation * qyaw * qpitch;
	transform.m_rotation = glm::normalize(transform.m_rotation);

	m_data.distance = m_data.distance + translate.z;
	transform.m_position = (transform.m_rotation * glm::vec3(0.0f, 0.0f, m_data.distance)) + m_target;
	glm::vec3 up = transform.m_rotation * glm::vec3(0.0f, 1.0f, 0.0f);
	m_mxView = glm::lookAt(transform.m_position, m_target, up);
}
